...
public class ChefController {
	...
	public @ResponseBody String processDChef(@RequestBody String body, HttpServletRequest request,
			HttpServletResponse response) {
		String jsonResponse = null;
		try {
			System.out.println("request : " + body);
	...
	public @ResponseBody Object getMealMenus(
	...
	public @ResponseBody Object getMealMenu(
			@RequestParam(value = "currentDate") @DateTimeFormat(iso = ISO.DATE) Date curDate,
			HttpServletRequest request, HttpServletResponse response) {
		if (curDate != null) {
	...
	public @ResponseBody Object updateMealMenu(@RequestBody MealMenu mealMenu, HttpServletRequest request,
			HttpServletResponse response) {
		if (mealMenu != null) {
	...
	public @ResponseBody Object createMealMenu(@RequestBody MealMenu mealMenu, HttpServletRequest request,
			HttpServletResponse response) {
		if (mealMenu != null) {
	...
	public void login(@RequestParam(value = "inputID", required = true) String id,
		...
		if (id != null && passwd != null && id.length() > 0 && passwd.length() > 0) {
			if (id.trim().equalsIgnoreCase("admin") && passwd.trim().equalsIgnoreCase("1234")) { // admin/1234로 로그인
		...
		} else { // id나 passwd가 하나라도 null이거나 의미없는 값이 있다면
			try {
